...
def _(inp, db: object, model: object):
    ids = [
        item[0]
        for item in db.session.query(model.main).filter(model.refer.in_(inp)).all()
    ]
    return ids
...
def _(inp, db: object, model: object):
    ids = [
        item[0]
        for item in db.session.query(model.main).filter(model.refer == int(inp)).all()
    ]
    return ids
...
def filter_by_kw(inp: str, db: object, model: object = Text2key) -> list:
    ...
    for k in kw_list:
        ...
        indices = (
            db.session.query(model.main).filter(model.refer.in_(sub.subquery())).all()
        )
    ...
    if len(t_ids) == 0:
        return []
    text_inds = [
        item[0] for item in db.session.query(Texts.id).filter(Texts.id.in_(t_ids)).all()
    ]
    return text_inds
...
mapping = {
    ...
    "vill_inf": {
        ...
        "model": {"relation": Inf2vill, "main": Text2inf},
    },
    "q_list": {
        "filter": filter_by_relation,
        "model": {"relation": Question2ql, "main": Text2quest},
    },
    "ray": {
        "filter": filter_by_relation,
        "model": {"relation": Vill2ray, "main": Text2vill},
    },
}
...
def query_wrapper(db: object, paginate: bool, **kwargs):
    ...
    for key in new_context.keys():
        if key == "page" or key == "download":
            continue
        result = set(
            mapping[key]["filter"](new_context[key], db, mapping[key]["model"])
        )
        if len(result) == 0:
            return abort_empty()
    ...
    if paginate:
        filtered = (
            db.session.query(Texts)
            .filter(Texts.id.in_(list(id_intersection)))
            .paginate(page=new_context["page"], per_page=PAGINATION_ITEMS)
        )
    else:
        filtered = (
            db.session.query(Texts).filter(Texts.id.in_(list(id_intersection))).all()
        )
